...
def compute_delta_p(Rot, p):
    ...
    for k in range(0, k_max):
        ...
        delta_p = (
            Rot[idxs_0]
            .transpose(-1, -2)
            .matmul(((p[idxs_end] - p[idxs_0]).float()).unsqueeze(-1))
            .squeeze()
        )
        list_rpe[2] = delta_p
    return list_rpe
...
def train_filter(args, dataset):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        save_iekf(args, iekf)
        print(
            "Amount of time spent for 1 epoch: {}s\n".format(
...
def prepare_loss_data(args, dataset):
    ...
    for dataset_name, rpe in list_rpe_.items():
        if len(rpe[0]) is not 0:
            ...
            cprint(
                "%s has too much dirty data, it's removed from training list"
                % dataset_name,
                "yellow",
            )
    ...
    for dataset_name, rpe in list_rpe_validation_.items():
        if len(rpe[0]) is not 0:
            dataset.list_rpe_validation[dataset_name] = list_rpe_validation[
                dataset_name
            ]
        else:
            ...
            cprint(
                "%s has too much dirty data, it's removed from validation list"
                % dataset_name,
                "yellow",
            )
    mondict = {
...
def train_loop(args, dataset, epoch, iekf, optimizer, seq_dim):
    ...
    for i, (dataset_name, Ns) in enumerate(dataset.datasets_train_filter.items()):
        t, ang_gt, p_gt, v_gt, u, N0 = prepare_data_filter(
            dataset, dataset_name, Ns, iekf, seq_dim
        )

        loss = mini_batch_step(
            dataset,
            dataset_name,
            iekf,
            dataset.list_rpe[dataset_name],
            t,
            ang_gt,
            p_gt,
            v_gt,
...
def mini_batch_step(
    dataset, dataset_name, iekf, list_rpe, t, ang_gt, p_gt, v_gt, u, N0
):
    iekf.set_Q()
...
def precompute_lost(Rot, p, list_rpe, N0):
    ...
    if len(idxs_0_bis) is 0:
        ...
        delta_p = (
            Rot_10_Hz[idxs_0_bis]
            .transpose(-1, -2)
            .matmul((p_10_Hz[idxs_end_bis] - p_10_Hz[idxs_0_bis]).unsqueeze(-1))
            .squeeze()
        )
