...
def results_filter(args, dataset):
    ...
    for i in range(0, len(dataset.datasets)):
        ...
        (
            ...
            t_c_i,
            measurements_covs,
        ) = dataset.get_estimates(dataset_name)
        ...
        v_r_gt = (
            Rot_gt.transpose(-1, -2)
            .matmul(torch.from_numpy(v_gt).float().unsqueeze(-1))
            .squeeze()
        ).numpy()

        p_r = (RotT.matmul(torch.from_numpy(p).float().unsqueeze(-1)).squeeze()).numpy()
        p_bis = (
            Rot_gt.matmul(torch.from_numpy(p_r).float().unsqueeze(-1)).squeeze()
        ).numpy()
        error_p = p_gt - p_bis
        ...
        axs1[2].set(
            xlabel="time (s)",
            ylabel="$\mathbf{R}_n^T \mathbf{v}_n$ (m/s)",
            title="Velocity in body frame",
        )
        axs2[0].set(
            xlabel="time (s)",
            ylabel=r"$\phi_n, \theta_n, \psi_n$ (rad)",
            title="Orientation",
        )
        axs2[1].set(
            xlabel="time (s)",
            ylabel=r"$\mathbf{b}_{n}^{\mathbf{\omega}}$ (rad/s)",
            title="Bias gyro",
        )
        axs2[2].set(
            xlabel="time (s)",
            ylabel=r"$\mathbf{b}_{n}^{\mathbf{a}}$ (m/$\mathrm{s}^2$)",
        ...
        ax4.set(
            xlabel=r"$p_n^x$ (m)",
            ylabel=r"$p_n^y$ (m)",
            title="Aligned position on $xy$",
        )
        axs5[0].set(
            xlabel="time (s)",
            ylabel=r" $\mathrm{cov}(\mathbf{y}_{n})$ (log scale)",
            title="Covariance on the zero lateral and vertical velocity measurements (log "
            "scale)",
        )
        axs5[1].set(
            xlabel="time (s)",
            ylabel=r"Normalized gyro measurements",
            title="Normalized gyro measurements",
        )
        axs5[2].set(
            xlabel="time (s)",
            ylabel=r"Normalized accelerometer measurements",
            title="Normalized accelerometer measurements",
        )
        axs6[0].set(
            xlabel="time (s)",
            ylabel=r"$\omega^x_n, \omega^y_n, \omega^z_n$ (rad/s)",
            title="Gyrometer",
        )
        axs6[1].set(
            xlabel="time (s)",
            ylabel=r"$a^x_n, a^y_n, a^z_n$ (m/$\mathrm{s}^2$)",
            title="Accelerometer",
        )
        axs7[0].set(
            xlabel="time (s)",
            ylabel=r"$|| \mathbf{p}_{n}-\hat{\mathbf{p}}_{n} ||$ (m)",
        ...
        axs7[2].set(
            xlabel="time (s)",
            ylabel=r" $\mathbf{\xi}_{n}^{\mathbf{p}}$",
